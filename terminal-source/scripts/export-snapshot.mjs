...
async function seed(snapshot){if(snapshot?.schemaVersion!==1)throw Error('Invalid seed snapshot');for(const [name,key] of [['cache.json','papers'],['conference-cache.json','conferences'],['blog-cache.json','blogs'],['affiliations-cache.json','affiliations']])await writeFile(new URL('data/'+name,root),JSON.stringify(snapshot[key]));}
