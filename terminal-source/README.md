...
## Conference coverage
Desk and Conferences default to 27 curated venues grouped by security/privacy, AI/language, multimodal/audio, and specialist research. `shared/venues.js` defines this editorial list. All indexed venues remains available, with separate filters for main proceedings, Findings, workshops and other/unknown tracks. Labels follow deposited metadata, not a manual acceptance check. Conference lists and date windows use proceedings publication dates, latest first; merged preprints retain their original dates for research trends. Month/year-only dates keep their precision.

`/api/conferences` refreshes hourly and retains up to 3,000 records, searching from January 1 two calendar years ago through today. OpenAlex queries (up to 200 each) target named ML/NLP/vision/security source indexes and broader conference-paper metadata. Crossref queries (up to 100 each) search proceedings articles around hallucination, adversarial research, privacy and ransomware. Additional venue-targeted Crossref queries retrieve up to 100 relevance-ranked metadata candidates per venue, then require an exact normalized venue match and safety/security relevance. PoPETs journal records are eligible. Candidate collection is sampled; display order is always by publication date unless the user selects oldest first. Each query reports its own status and matched count in settings.
...
`node check-conferences.mjs [URL]` checks the curated default, broader scope, track filters, proceedings-date ordering after merging, and mobile layout against a static snapshot.
