...
const relevant=/safety|secur|adversar|hallucinat|privacy|fairness|bias|interpretabil|alignment|unlearning|deepfake|watermark|personali[sz]|misinformation|robustness|malware|ransomware|vulnerabilit|phishing|jailbreak/i;
export function parseCrossref(data,now=new Date()){
 ...
 return data.message.items.flatMap(w=>{
  ...
  if(!title||!venueName||!relevant.test(title+' '+abstract))return[];
